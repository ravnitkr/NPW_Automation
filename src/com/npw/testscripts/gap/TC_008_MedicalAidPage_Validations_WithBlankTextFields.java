...
public class TC_008_MedicalAidPage_Validations_WithBlankTextFields extends BaseTest {
	...
	private static String TestCaseName = "TC_008_MedicalAidPage_Validations_WithBlankTextFields";
	...
	public static void GetAllDetailsFromResidentialPage()
		...
		Reporting.Testcasename = "Get all validations from the Medical Aid Page when textfields are kept blank";
		...
		try {
			...
			Reporting.logResults("Pass", "Enter/Fill the details in medical cover page",
					"Succesfully entered the details in Medical Aid page");
			
			bStatus = GapPageObject.continueButton();

			// Get all validations
			bStatus = GapPageObject.getAllValidationMessages("MedicalAidDetailsBLANKPage");
			if (!bStatus) {
				Reporting.logResults("Fail", "Get all validations on great decision step.",
						"Unable to get all validations on  Medical Aid page due to" + Messages.errorMsg);
				return;
			}
			Reporting.logResults("Pass", "Get all validations on Medical Aid step.",
					"All validations successfully matched on Medical Aid Step");
